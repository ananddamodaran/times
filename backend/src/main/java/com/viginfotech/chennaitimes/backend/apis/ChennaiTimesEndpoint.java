...
public class ChennaiTimesEndpoint {
    ...
    public Feed getNewsDetail(@Named("source")int source, @Named("category")int category, @Named("guid")String guid){
        ...
        if(feed.getDetailNews()!=null) return feed;
            ...
            switch (source){
                case SOURCE_DINAKARAN: return Dinakaran.getDinakaranDetail(guid);
                case SOURCE_DINAMALAR : return  Dinamalar.getDetail(guid,category);
                case SOURCE_BBCTAMIL: return BBCTamil.getDetail(guid);
                case SOURCE_DINAMANI: return Dinamani.getDinamaniDetail(guid);
                case  SOURCE_ONEINDIA : return OneIndia.getOneIndiaDetail(guid);
                case  SOURCE_NAKKHEERAN : return Nakkheeran.getNakkheeranDetail(guid);
                default: return null;
            }
        }
